...
public class Board {
	/**
	 * Klasa służy do przechowywania informacji na temat stanu planszy oraz
	 * rozgrywki z puktu widzenia danego gracza. Jest wykorzystywana zarówno przez
	 * klienta jak i przez serwer.
	 */
	private char[][] board;
	...
	public boolean changeBoard(int row, int col, char mark) {
		if (row >= 0 && row <= 2 && col >= 0 && row <= 2 && board[row][col] == ' ') {
			board[row][col] = mark;
			return true;
		}
		return false;
	...
	public int checkForWin() {
		/*
		 * Zwracane wartości: 0 - nie koniec 1 - wygrana 2 - przegrana 3 - remis
		 */
